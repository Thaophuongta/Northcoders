...
function getCharCode(cha) {
  const code = cha.charCodeAt()
return `The ASCII code for ${cha} is ${code}`
}
...
if (level <= 5) {return `Warning - battery level low: ${level}%, please charge your device`}
else if ((level > 5) && (level <= 99)) {return `Battery level: ${level}%`}
else {return 'Fully charged :)'}
}
...
for (let i = 0; i < array.length; i++) {
  if (typeof array[i] === 'string') {result.push(array[i])}
}
return result
}
